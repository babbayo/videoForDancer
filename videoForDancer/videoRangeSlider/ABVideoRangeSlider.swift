...
//    }
    ...
    public func updateThumbnails(){
        if !isUpdatingThumbnails{
            ...
            backgroundQueue.async {
                _ = self.thumbnailsManager.updateThumbnails(view: self, asset: self.asset, duration: self.duration)
    ...
    public func setProgressPosition(seconds: Float){
        self.progressPercentage = self.valueFromSeconds(seconds: seconds)
        layoutSubviews()
    }
    
    public func getStartPosition( ) -> Float64 {
        return secondsFromValue(value: self.startPercentage)
        
    }

    public func getEndPosition() -> Float64 {
    ...
    private func processHandleDrag(
        ...
        if drag == .start {
            progressPosition = positionFromValue(value: max(self.startPercentage, self.progressPercentage))
        } else {
            progressPosition = positionFromValue(value: min(self.endPercentage, self.progressPercentage))
        }
